...
abstract class Entity extends \Core\Component {
  /**
   * Configures validations in the given form according to the current Entity
   * extender. This is useful to keep validation information in the model.
   *
   * @param \Symfony\Component\Form\FormBuilderInterface $form_builder
   * @param null $fields
   *  If specified, only applies validations to the specified fields.
   */
  static function setupFormValidation(&$form_builder, $fields = NULL) {
...
//				}
  ...
  public function collectValidationErrors() {
    if ($this->_collect_validation_errors) {
      throw new Exception('Already collecting validation errors');
    } //TODO: change
  ...
  public function checkValidation() {
    if (!$this->_collect_validation_errors) {
      throw new Exception('Not collecting validation errors. Call collectValidationErrors() first');
    } //TODO: change
    ...
    if ($validators) {
      ...
      if (!$v->isValid($value)) {
        if ($collect_errors) {
          ...
          throw new Exception('Validation errors: ' . print_r(array(
              $name,
              $value,
              $v->getMessages()
            ), TRUE));
...
//				}
    ...
    if ($value == NULL && $this->getReferenceId($name)) //clear reference
    {
  ...
  public function setCollection($name, $class_name, $fk_field = NULL) {
    return $this->data[$name] = new CModel_Collection($class_name, $fk_field, $this);
  }
